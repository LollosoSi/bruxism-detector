...
public class LinearSVMExact {
    ...
    public static void main(String[] args) throws Exception {
        double[][] clenching = readCSV("clenching.csv");
        double[][] nonClenching = readCSV("non_clenching.csv");
        ...
        LinearSVMExact svm = new LinearSVMExact();
        svm.fit(X, y);

        double[] w = svm.getWeights();
        double b = svm.getBias();

        System.out.println("\n// Copy to Arduino sketch:");
        System.out.print("static const float weights[] = { ");
        for (int i = 0; i < w.length; i++) {
            System.out.printf("%.8f%s", w[i], (i < w.length - 1) ? ", " : " ");
        }
        System.out.println("};");
        System.out.printf("static const float bias = %.8f;%n", b);
		...
		System.out.printf("static const int classification_threshold = %.0f;%n", threshold);
    }
}
